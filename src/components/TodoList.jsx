    ...
    <section className="bg-blue">
      <div className="container mx-auto">
        <div className="text-center  pt-24">
          <div className="inline-block bg-white border-2 p-6 rounded-lg">
            ...
            <div className="flex justify-center gap-3">
              <button
                className="border border-[3px] border-red-800 text-black px-4 py-2 text-center rounded-2xl hover:bg-red-800 hover:text-white transition duration-300"
                onClick={() => setFillter("all")}
              >
                All
              </button>
              <button
                className="border border-[3px] border-yellow-500  text-black px-3 py-2 text-center rounded-3xl hover:bg-yellow-500 hover:text-white transition duration-300"
                onClick={() => setFillter("pending")}
              >
                Pending
              </button>
              <button
                className="border border-[3px] border-green-600 text-black px-3 py-2 text-center rounded-3xl hover:bg-green-600 hover:text-white transition duration-300"
            ...
            <div className="mt-6 h-full max-h-80  relative overflow-y-auto shadow-md sm:rounded-lg">
              <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                  <tr>
                    <th className="p-4"></th>
                    <th className="px-6 py-3 text-white">Task</th>
                  ...
                  {tasks.length === 0 ? (
                    ...
                    fillterarr.map((taskItem) => (
                      ...
                      >
                        ...
                        {taskItem.isComplete ? <td className="px-6 py-4 line-through font-medium text-gray-500">
                          {taskItem.task}
                        </td> : <td className="px-6 py-4  font-medium text-gray-900">
                          {taskItem.task}
                        </td>}
